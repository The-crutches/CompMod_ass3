...
```{r step 3}
#Building model 1 (do we need to add language??????)
f1 <- bf(Pitch_IQR ~ 1 + Diagnosis + (1|ID))

#Getting priors 
get_prior(f1, data = df)

#Setting priors  ##SHOULD IT BE MORE SCEPTICAL??
f1_prior <- c(
  prior(normal(0,1), class = Intercept), #Mean of 0 and sd of 1 (we have standardized the data)
  prior(normal(1,.5), class = sd), #Mean of 1 sd of .5 (why did we do that????)
...
#Better pp_check (Does this make sense to include????)
...
f2_prior <- c(
  prior(normal(0,1), class = Intercept), #Still standardized data
  prior(normal(1,.32), class = sd), #adding the population effects (IS THIS RIGHT????)
...
```{r step 5}
#WTF THIS DOESNT WORK ANYMORE 
#Comparing the models with waic
m1 <- add_criterion(m1, "waic")
m2 <- add_criterion(m2, "waic")

w_waic <- loo_compare(m1, m2, criterion = "waic")

print(w, simplify = F)

###DOES THIS DO ANYTHING SARAH???
...
w_loo <- loo_compare(m2, m4, criterion = "loo")


loo_model_weights(m2,m4)
