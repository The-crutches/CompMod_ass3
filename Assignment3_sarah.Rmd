...
library(pacman)
p_load(readr, tidyverse, metafor, brms, dplyr, rethinking, MASS, psych)
...
# Making the formula with a weighted average. telling the mdodel to not take the effectsize at face value, but to weight it based onn the SE. also telling it that the otcome is not one value but a distribution. 
...
random slope: Gender (but be aware that we are making assumptions about the effect of pitch_var, that is, that pitch is variable differently in )
random intercept: ID
...
hypothesis(m4, "Intercept > DiagnosisTD")
