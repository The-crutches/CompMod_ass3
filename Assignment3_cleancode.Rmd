...
#Setting the meta analytic priors #During the lecture we realized that we set the priors wrong - we have the priors for sd in sigma and the other way around. 
...
## The correct priors we should have used (added after the lecture 22-04)
f2_prior <- c(
  prior(normal(0,.5), class = Intercept),
  prior(normal(.32,.2), class = sd),#between study heterogeneity we broadened the SD from 0.1 tol 0.2 to include between partcipant variation and not just between study variation as before.  
  prior(normal(-0.43,0.1), class = b, coef = DiagnosisTD), # putting the MA prior here - difference between groups 
  prior(normal(0, 0.5), class = sigma) # family normal therefore mean of 0. the sd indicate the error that is left after dealing within between partcipant variation. 
)
...
w_loo
  #Model 1 seems like the best, though pareto k values are too high

#Loo weights 
loo_model_weights(m1,m2)
    #Model 1 is weighted 1 
pp_average(m1, m2)
```

```{r}
#Exploring students t 
...
#Better check
y_pred4 <- posterior_linpred(m3_prior)
dens(y_pred4)
...
#Checking chains
color_scheme_set("viridis")
mcmc_trace(m3,  pars = "b_Intercept", "b_DiagnosisTD") + theme_classic()
mcmc_rank_overlay(m3, pars = "b_Intercept", "b_DiagnosisTD") + theme_classic()

#Hypothesis checking
hypothesis(m3, "DiagnosisTD < 0")

#Posterior update check
plot(hypothesis(m3, "DiagnosisTD < 0"))
...
w_loo <- loo_compare(m1, m2, m3, criterion = "loo")
