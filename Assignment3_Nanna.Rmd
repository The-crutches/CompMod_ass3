...
#Takes dataset(MA_d), want to know what kind of effect size (standardized mean difference), nunber og participant in each group (TD_N and ASD_N), Mean of the variable we are calculating for each group (PitchVariability), and SD for both groups, slab is for plotting? 
...
# Making a Meta analytic formula - bayesian - whet the effect size with a given certainty with an intercept and the varying effect is by paper - Ricc changes the effect to population because some of the studies used the same population - we might also add language
